...
class BotScoreDumpTest {
    @Test
    fun dump() {
        SuperiorBotEngine.weights = BotRewardWeights(debugLogging = true, expectimaxDepth = 2, maxDecisionMillis = 50)
        val captureCase = listOf(
            MatchPlayerState("b", "Bot", "green", true, listOf(0, 50, -1, -1)),
            MatchPlayerState("h", "Human", "yellow", false, listOf(42, -1, -1, -1)),
        )
        SuperiorBotEngine.chooseToken(captureCase, 0, listOf(0, 1), 3, BotDifficulty.HARD, null, Random(12))

        val homeCase = listOf(
            MatchPlayerState("b", "Bot", "green", true, listOf(50, 5, -1, -1)),
            MatchPlayerState("h", "Human", "yellow", false, listOf(-1, -1, -1, -1)),
        )
